...
import { buildProp, definePropType, isBoolean, isClient } from '@ls-ui/utils';

import type { ExtractPropType } from '@ls-ui/utils';
...
export const createModelToggleComposable = <T extends string>(name: T) => {
  ...
  const useModelToggle = ({
    indicator, // popper是否展示
    toggleReason, // event事件
    shouldHideWhenRouteChanges, // 路由切换是否隐藏popper
    shouldProceed, // 是否应该继续操作
    onShow, // 执行展示popper的函数
    onHide, // 执行隐藏popper的函数
  }: ModelToggleParams) => {
